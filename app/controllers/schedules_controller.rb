class SchedulesController < ApplicationController
  ...
  def create
    ...
    @schedule.restaurant = current_user.restaurants.find(params[:resto_id])
    if Schedule.check_overlapping?(schedule_params, @schedule.restaurant) && @schedule.save
  ...
  def update
    if Schedule.check_overlapping?(schedule_params, @schedule.restaurant) && @schedule.update(schedule_params)
      ...
      @schedule.errors.full_messages.map { |msg| flash[:failure] << "<dd>#{msg}</dd>" }
      flash[:failure] << "<dd>Overlapping Schedules</dd>" unless Schedule.check_overlapping?(schedule_params, @schedule.restaurant)
      flash[:failure] << "</dl>"
